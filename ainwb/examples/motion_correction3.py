...
settings = {}
settings["filename"] = "sample_motion_correction_3.nwb"
...
settings["identifier"] = nwb.create_identifier("motion correction example 3")
...
settings["start_time"] = "Tue Jan 14 2016 11:55:20"

# provide one or two sentences that describe the experiment and what
#   data is in the file
settings["description"] = "Test file demonstrating use of the MotionCorrection interface with two channels on the same microscope"
...
neurodata.set_metadata(IMAGE_SITE_EXCITATION_LAMBDA("camera1"), "950nm, 1000 nm")
neurodata.set_metadata(IMAGE_SITE_INDICATOR("camera1"), "GCaMP6t, tdTomato")
neurodata.set_metadata(IMAGE_SITE_DEVICE("camera1"), "Acme 2-photon microscope")
# channel 1
neurodata.set_metadata(IMAGE_SITE_CHANNEL_LAMBDA("channel1"), "950nm")
neurodata.set_metadata(IMAGE_SITE_CHANNEL_DESCRIPTION("channel1"), "image cells expressing GCaMP6t")
# channel 2
neurodata.set_metadata(IMAGE_SITE_CHANNEL_LAMBDA("channel2"), "1020nm")
neurodata.set_metadata(IMAGE_SITE_CHANNEL_DESCRIPTION("channel2"), "image cells expressing tdTomato")
...
#
# this example uses 2 channels. the image stacks and motion-corrected data
#   is stored independently for each camera. the code for storing data from
#   channel 2 is almost identical to that of channel 1 in this example.
...
orig = neurodata.create_timeseries("TwoPhotonSeries", "Channel1's source image", "acquisition")
...
# specify the file
orig.set_value("external_file", "/path/to/file/channel1_stack.tif")
...
orig.set_value("channel_name", "channel1")
...
corr.set_value("external_file", "/path/to/file/corrected_channel1_stack.tif")
...
mod = neurodata.create_module("Channel1")
...
iface.set_value("random comment", "Note that the 'original' field under 2photon is an HDF5 link to the time series '/acquisition/timeseries/Channel1's source image'")
...
#
# add image stack
orig = neurodata.create_timeseries("TwoPhotonSeries", "Channel2's source image", "acquisition")
orig.set_description("Pointer to a 640x480 image stored in the file system")
orig.set_source("Data acquired from Acme 2-photon microscope")
# assume the file is stored externally in the file system
orig.set_value("format", "external")
# specify the file
orig.set_value("external_file", "/path/to/file/channel2_stack.tif")
# information about the image
orig.set_value("bits_per_pixel", 16)
orig.set_value("dimension", [640, 480])
###########################
...
# imaging plane value is the site/imaging-plane defined as metadata above
orig.set_value("imaging_plane", "camera1")
orig.set_value("channel_name", "channel2")
orig.set_value("scan_line_rate", 16000)
###########################
# store time -- this example has 3 frames
orig.set_time([0, 1, 2])
...
corr.set_value("format", "external")
corr.set_value("external_file", "/path/to/file/corrected_channel2_stack.tif")
corr.set_value("bits_per_pixel", 16)
corr.set_value("dimension", [640, 480])
corr.set_time_as_link(orig)
corr.set_value("num_samples", 3)
...
# create the module and MotionCorrection interface
mod = neurodata.create_module("Channel2")
# the module can store as many interfaces as we like. in this case we
#   only have one. create it
iface = mod.create_interface("MotionCorrection")
# add the time series to the interface
...
iface.set_value("a comment", "Note that the 'original' field under 2photon is an HDF5 link to the time series '/acquisition/timeseries/Channel2's source image'")
